...
`container` 와 `components` 로 컴포넌트를 구분하는 방식은 사실 실제로는 잘 쓰이지 않는 방식이다. 내가 겪었던 문제와 같이, 프로젝트 규모가 커질수록 이 방식으로 모든 컴포넌트를 규정하기는 어렵기 때문이다. 리액트에서 상태를 관리하고 전달하는 매커니즘에 대해 익히기 위한 목적으로 간단한 예제를 만들어보는 경우라면 이 방식을 실습해 보는 것이 도움이 된다. 하지만 실제 프로젝트를 진행할 때는 프로젝트에 맞는 폴더 구조를 고민해 보고 적용하는 것이 좋겠다.