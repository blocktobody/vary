...
내가 리액트를 처음 배울 때는 Hook과 함수형 컴포넌트가 나온 지 얼마 되지 않을 때였다. 클래스로 컴포넌트를 구현하고, 그 안에서 리액트 라이프사이클을 활용하여 컴포넌트 내의 동작을 제어하는 방법이 정석이었다. 그러나 회사에서 실무를 하기 시작했을 때 개발팀의 의사 결정에 따라 함수형 컴포넌트 위주로 개발하게 되었고 이내 이 방식에 익숙해져버렸다.

함수형 컴포넌트는 클래스 컴포넌트에 비해 작성하기가 간편하면서도 동시에 클래스 컴포넌트에서 지원되던 기능들을 모두 쓸 수 있기 때문에 익숙해지고 나면 대부분의 경우에는 굳이 클래스 컴포넌트로 돌아갈 필요가 없어진다. 그러나 요컨대 Syntactic Sugar로 인해 개발 효율이 늘어나는 것은 사실이지만, 어째서 그런 요령이 필요하게 되었는지를 제대로 이해하지 않고 남발했을 때는 예상치 못한 문제들을 맞닥뜨리게 된다. 그중에서도 `useEffect` Hook이 주는 편리함과 그에 가려 잘 알려고 하지 않았던 함정들을 한번 정리해 보고자 한다.

### 1) 라이프 사이클 대체하기

`useEffect` 라는 이름이 다소 모호하긴 하지만 [공식문서](https://reactjs.org/docs/hooks-effect.html)를 자세히 읽어 보면 그 쓰임을 잘 이해할 수 있다. 또한 기존의 클래스 컴포넌트 라이프 사이클을 이해하고 있었다면 더욱 `useEffect` Hook의 필요성에 대해 공감할 수 있을 것이다. 공식문서에 따르면 `useEffect` 의 기본적인 사용 방법은 다음과 같다.
...
이렇게 작성하면 해당 `useEffect` 훅은 컴포넌트가 처음 렌더됐을 때에만 동작할 것이다. `useEffect` 를 사용하는 많은 경우에는 dependency에 비었거나 혹은 변수가 담긴 배열을 넘겨주는 것이 일반적이다. 컴포넌트 내의 어떤 한 부분이라도 변화가 있을 시 리렌더되는 리액트의 특성상, 리렌더가 너무 많이 발생하면 `useEffect` 가 실제 필요와는 상관 없이 과도하게 실행될 수 있기 때문이다. dependency 배열이 파라미터로 주어지면 `useEffect` 는 해당 배열 안의 변수가 변했을 때에만 해당 로직을 실행한다. 
