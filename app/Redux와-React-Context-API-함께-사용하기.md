...
처음 리액트를 배웠을 때는 자바스크립트는 물론 코딩 자체에 입문하는 단계였다. 도움을 받아야겠다 싶어서 강의를 들었다. 첫 시간부터 상태 관리라는 것이 왜 필요한지도 모르면서 `this.state` 를 따라 쓰고, 또 `this.setState` 를 따라 쓰고. 그러다가 작은 것이라도 직접 만들어 봐야 할 필요성을 느꼈다. 그래서 누구나 시작할 때 한 번씩 만들어 본다는 투두 리스트([링크](https://codesandbox.io/s/autumn-dew-pmq8h?fontsize=14&hidenavigation=1&theme=dark))를 만들었다. 물론 0에서부터 직접 만든 것이 아니라 예제를 보고 따라했다. 그런데 프로젝트 구조를 보면 나와있듯이 리덕스를 적용한 예제였다. 상태 관리의 개념부터가 난관이었던 주제에 갑자기 웬 리덕스? 싶겠지만 내가 그 개념을 이해하게 된 계기가 바로 이 예제 덕분이었다.

### Redux로 상태를 관리하는 방법

상태 관리가 왜 필요한지부터 정리해 보자. 상태 관리는 동적 페이지를 만들기 위한 시작이다. 예를 들어 어떤 내용을 입력한 후 버튼을 눌러 제출하는 아주 사소한 기능을 구현하는 데 있어서도 상태 관리가 필요하다. 즉 사용자의 행동으로 인한 변화를 페이지에 반영하기 위해서는 어떤 변화가 있었는지를 기록하고 그 중 필요한 내용을 표시할 수 있어야 한다는 뜻이다. 이 개념을 투두 리스트에 적용하면 어떻게 될까? 우선 투두 리스트에서 요구되는 기본적인 기능은 다음의 세 가지가 있다.
...
`Context API` 를 함께 사용하면 리덕스의 복잡한 스토어 문제도 해결된다. 프로젝트 전체적으로 공유할 필요가 있는 상태들에 한에서만 리덕스의 글로벌 스토어를 사용하고, 특정 부분에서만 공유하는 상태들의 경우에 컨텍스트를 사용하면 된다.

### 비동기 처리를 구현하려면

이렇게 상태 관리의 개념을 이해하고 적절한 방법으로 구현하는 것까지 성공했다. 그런데 만약 상태의 변화가 일련의 과정을 따라 순차적으로 일어나야 하는 경우라면 어떻게 될까? 여기서 비동기 처리에 대한 필요성이 생겨난다. 배리 프로젝트에서는 이를 구현하기 위해 `redux-observable` 과 이의 기반 라이브러리인 `rxjs` 를 활용했다. 이에 대한 자세한 내용을 다음 글에서 정리해보기로 하겠다.